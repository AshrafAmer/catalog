...
# Connect to client_secrete.json File
...
def gconnect():
    ...
    if result['issued_to'] != CLIENT_ID:
        response = make_response(json.dumps("ID does not match app's."), 401)
...
def editRestaurant(restaurant_id):
    ...
    if editedRestaurant.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You can not edit.');}" \
...
def showMenu(restaurant_id):
    ...
    c_id = creator.id
    if 'username' not in login_session or c_id != login_session['user_id']:
        return render_template(
...
def newMenuItem(restaurant_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        return "<script>function myFunction() {alert('You can not add.');}" \
               "</script><body onload='myFunction()''>"
